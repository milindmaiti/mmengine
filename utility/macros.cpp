...
namespace BitUtil {
  ...
  ull decode_dst(ull move) { return ((move) & 0xFC0) >> 6; }
  ull decode_piece(ull move) { return ((move) & 0xF000) >> 12; }
  ull decode_promotion(ull move) { return ((move) & 0xF0000) >> 16; }
  ull decode_castle(ull move) { return ((move) & 0x100000) >> 20; }
  ull decode_capture(ull move) { return ((move) & 0x200000) >> 21; }
  ull decode_double_push(ull move) { return ((move) & 0x400000) >> 22; }
  ull decode_en_passant(ull move) { return ((move) & 0x800000) >> 23; }

  ull encode_move(ull src, ull dst, ull piece, ull promotion, ull castle,
                  ull capture, ull doublePush, ull enPassant) {
    return (src) + (1ULL << 6) * (dst) + (1ULL << 12) * (piece) +
           (1ULL << 16) * (promotion) + (1ULL << 20) * (castle) +
           (1ULL << 21) * (capture) + (1ULL << 22) * (doublePush) +
           (1ULL << 23) * (enPassant);
  }
  // bit manipulation templates are defined inline in macros.h
} // namespace BitUtil
...
extern const std::array<int, Constants::NUM_SQ> squareMirror = {
  ...
  extern const std::array<int, Constants::ASCII_SZ> charPieces = [] {
    std::array<int, Constants::ASCII_SZ> arr{};
    arr['P'] = P;
    arr['N'] = N;
    arr['B'] = B;
    arr['R'] = R;
